...
namespace Inventory.Editor.Drawers
{
    ...
    public class ItemTablePropertyDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            var itemTable = PropertyUtils.GetValue<ItemTable>(property, fieldInfo);

            if (itemTable == null)
            {
                EditorGUI.TextField(position, property.displayName, "null");
            ...
            var currentGridTableProperty =
                PropertyUtils.FindRelativeAutoProperty(property, nameof(ItemTable.CurrentGridTable));
            ...
            EditorGUI.PropertyField(new Rect(gridPosYRect.x, gridPosYRect.y, gridPosYRect.width, LineSpace),
                PropertyUtils.FindRelativeAutoProperty(property, nameof(ItemTable.CurrentItemHolder)),
                true);

            gridPosYRect.y += LineSpace + MarginTopField;

            EditorGUI.PropertyField(new Rect(gridPosYRect.x, gridPosYRect.y, gridPosYRect.width, LineSpace),
                PropertyUtils.FindRelativeAutoProperty(property, nameof(ItemTable.InventoryMetadata)),
                true);
        ...
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            ...
            var grid = property.FindPropertyRelative(
                PropertyUtils.ParseAutoPropertyName(nameof(ItemTable.CurrentGridTable)));
            if (grid != null)
            {
            ...
            var inventoryMetadataProperty =
                property.FindPropertyRelative(PropertyUtils.ParseAutoPropertyName(nameof(ItemTable.InventoryMetadata)));
