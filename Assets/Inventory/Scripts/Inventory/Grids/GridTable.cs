...
namespace Inventory.Scripts.Inventory.Grids
{
    ...
    public class GridTable
    {
        ...
        public InventoryMessages PlaceItem(ItemTable item, int posX, int posY)
        {
            if (!this.BoundaryCheck(posX, posY, item.Width, item.Height))
            {
                return InventoryMessages.OutOfBounds;
            }

            if (!OverlapCheck(posX, posY, item.Width, item.Height))
            {
                return InventoryMessages.Overlapping;
            }

            if (IsInsertingInsideYourself(item))
            {
                return InventoryMessages.InsertInsideYourself;
            }
            ...
            return InventoryMessages.Inserted;
        }
        ...
        public ItemTable[] GetAllContainersFromGrid()
        {
            ...
            foreach (var inventoryItem in InventoryItemsSlot)
            {
                ...
                if (inventoryItem.InventoryMetadata != null && inventoryItem.InventoryMetadata is ContainerMetadata)
                {
