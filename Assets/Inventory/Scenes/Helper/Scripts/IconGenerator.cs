...
namespace Inventory.Scenes.Helper.Scripts
{
    public class IconGenerator : MonoBehaviour
    {
        ...
        private void TakeScreenshot(string fullPath)
        {
            ...
            var messageDebug = "Pictures taking with " + GetPixels() + " pixels...";
            Debug.Log(messageDebug.Editor());

            var renderTexture = new RenderTexture(GetPixels(), GetPixels(), 24);
            _camera.targetTexture = renderTexture;
            var screenShot = new Texture2D(GetPixels(), GetPixels(), TextureFormat.RGBA32, false);
            _camera.Render();
            RenderTexture.active = renderTexture;
            screenShot.ReadPixels(new Rect(0, 0, GetPixels(), GetPixels()), 0, 0);
